...
CREATE TEMPORARY TABLE IF NOT EXISTS results (Dates VARCHAR(255), b_count INT(11), d_count INT(11), c_count INT(11), cu_count INT(11), bdc_count INT(11));
INSERT INTO results (Dates, b_count, d_count, c_count, cu_count, bdc_count)
select c.dates , 
        IFNULL(b.count,0), 
        IFNULL(d.count, 0), 
        IFNULL(c.count, 0),
        cu.count,               
        IFNULL(bdc.bdcounts, 0) 
from cumulative_value c 
left join bdq_containing_regimen b on c.dates=b.dates 
left join dlm_containing_regimen d on c.dates=d.dates 
left join cumulative_count cu on c.dates=cu.dates 
left join bdq_dlm_count bdc ON bdc.dates=cu.dates 
order by STR_TO_DATE(c.dates, "%Y/%M");

-- FINAL Results query
select Dates AS 'Year/Month', 
       b_count  AS 'Number of patients that started Bdq in a treatment for the first time that month', 
       d_count  AS 'Number of patients that started Dlm in a treatment for the first time that month', 
       c_count  AS 'Number of unique patients that enrolled in endTB that month (started either Bdq or Dlm, or Both)', 
       cu_count AS 'Cumulative number of unique patients that enrolled in endTB since start',   
       @concdummy:= @concdummy + bdc_count AS 'Cumulative number of unique patients that have gotten both Bdq and Dlm in a regimen' from results 
join (SELECT @concdummy:=0 AS concdummy) concdummy;
